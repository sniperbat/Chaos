...
namespace Chaos{
  ...
  //------------------------------------------------------------------------------------------------
  std::shared_ptr<ChsScene> ChsSceneManager::getScene( const std::string & className ){
    auto iter = this->scenes.find( className );
    if( iter != this->scenes.end() ){
      return iter->second;
    }
    std::shared_ptr<ChsScene> scene( sceneClassFactory->create( className ) );
    if( scene ){
  ...
  void ChsSceneManager::gotoScene( const std::string & name, bool isReleaseLastScene ){
    if( !this->lastScene.expired() ){
      if( this->lastScene.lock()->getName() == name ){
        ...
        const std::shared_ptr<ChsScene> & lastScene = this->lastScene.lock();
    ...
    std::shared_ptr<ChsScene> targetScene = this->getScene( name );
    if( targetScene ){//if target scene cannot be found, ignore it
      if( !this->currentScene.expired() ){
        const std::shared_ptr<ChsScene> & currentScene = this->currentScene.lock();
